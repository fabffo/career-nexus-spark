...
interface PrevisionVentesFormProps {
  annee: number;
}

export default function PrevisionVentesForm({ annee }: PrevisionVentesFormProps) {
  ...
  const { data: previsions = [] } = useQuery({
    queryKey: ["previsions-ventes", annee],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const addMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      ...
      const { error } = await supabase.from("previsions_ventes").insert({
        annee,
        mois: data.mois,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["previsions-ventes", annee] });
  ...
  const handleEdit = (prevision: any) => {
    ...
    setFormData({
      ...
      client_nom: prevision.client_nom || "",
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Prévisions de Ventes</h3>
        {!isAdding && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  {clients.map((client) => (
                    <SelectItem key={client.id} value={client.id}>{client.raison_sociale}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          {previsions.length === 0 ? (
            <TableRow>
              <TableCell colSpan={8} className="text-center text-muted-foreground py-8">
                Aucune prévision de vente pour {annee}
              </TableCell>
            </TableRow>
          ) : (
