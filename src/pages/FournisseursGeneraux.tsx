...
import { fournisseurGeneralService } from '@/services/contratService';
...
export default function FournisseursGeneraux() {
  ...
  const handleOpenForm = (fournisseur?: FournisseurGeneral) => {
    if (fournisseur) {
  ...
  const handleSubmit = async () => {
    try {
      if (selectedFournisseur) {
        await fournisseurGeneralService.update(selectedFournisseur.id, formData);
        toast.success('Fournisseur modifié avec succès');
      } else {
        await fournisseurGeneralService.create(formData);
  ...
  const handleDelete = async () => {
    if (selectedFournisseur) {
      try {
        await fournisseurGeneralService.delete(selectedFournisseur.id);
  ...
  const handleCopy = async (fournisseur: FournisseurGeneral) => {
    try {
      ...
      await fournisseurGeneralService.create(newFournisseur);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Fournisseurs Généraux</h1>
          <p className="text-muted-foreground mt-2">
            Gérez vos fournisseurs généraux
          </p>
        </div>
